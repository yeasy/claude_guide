...
程序化工具调用支持以下模型（均使用 `code_execution_20250825` 工具类型，并开启 `advanced-tool-use-2025-11-20` beta）：

| 模型 | 说明 |
| :--- | :--- |
| `claude-opus-4-7` | Opus 最新版（2026-04-16发布） |
| `claude-opus-4-6` | Opus 前版本 |
| `claude-sonnet-4-6` | Sonnet 最新版 |

该功能通过 Claude API、Claude Platform on AWS 和 Microsoft Foundry 提供；不等同于 Amazon Bedrock 或 Vertex AI 可用。
...
tools = [
    {
        "type": "code_execution_20250825",  # 代码执行工具
        "name": "code_execution"
    },
    {
        "name": "query_database",
...
| `["code_execution_20250825"]` | 仅允许从代码执行环境中调用 |
| `["direct", "code_execution_20250825"]` | 两种模式均可 |

> [!TIP]
> 建议为每个工具只选择 `['direct']` 或 `['code_execution_20250825']` 其中之一，这样能给 Claude 提供更明确的使用指引。
