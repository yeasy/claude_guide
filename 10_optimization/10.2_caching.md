...
> **2026 年 3 月更新**：Anthropic 于 2026 年 3 月将默认缓存 TTL 从 1 小时调整为 5 分钟。如需使用 1 小时缓存，必须在 `cache_control` 中显式指定 `"ttl": "1h"`。未指定 TTL 时默认为 5 分钟。这一变更可能影响高频调用场景的成本，建议根据实际访问模式选择合适的 TTL。
...
Claude Code 包含一个内部的 **YOLO 模式分类器**（auto-approve classifier），用于在自动批准模式下评估每次工具调用是否对齐用户意图、可否放行。该分类器与主对话共享缓存前缀，实现 **成本摊分**：
