...
Claude Code 是 Prompt Caching 大规模应用的最佳范例。在实际使用中，它实现了 **92% 的缓存命中率** 和 **81% 的成本削减**。理解它的缓存架构，对构建任何长会话智能体都有借鉴意义。
...
**关键数据**：Claude Code 中的 `DANGEROUS_uncachedSystemPromptSection()` 函数在某些条件下强制禁用缓存，这会导致 **12.5 倍的成本增长**。这个函数通常在安全敏感操作中被调用。
...
```javascript
const MODEL_CACHE_PRICING = {
  “claude-haiku-3-5”: {
    input_price: 0.80 / 1e6,           // 正常输入
    cache_write_price: 1.00 / 1e6,     // 缓存写入（+25%）
    cache_read_price: 0.08 / 1e6       // 缓存读取（-90%）
  },
  “claude-sonnet-4-0”: {
    input_price: 3.00 / 1e6,
    cache_write_price: 3.75 / 1e6,     // 写入 1.25x
    cache_read_price: 0.30 / 1e6       // 读取 $0.30 / MTok
  },
  “claude-opus-4-1”: {
    input_price: 15.00 / 1e6,
    cache_write_price: 18.75 / 1e6,
    cache_read_price: 1.50 / 1e6
  },
  “claude-opus-4-5”: {
    input_price: 15.00 / 1e6,
    cache_write_price: 18.75 / 1e6,
  ...
  “claude-opus-4-6”: {
    input_price: 3.00 / 1e6,           // Sonnet 价格
    cache_write_price: 3.75 / 1e6,
    cache_read_price: 0.30 / 1e6
  },
  “claude-opus-4-6-fast”: {
    input_price: 0.30 / 1e6,           // 快速模式
    cache_write_price: 0.375 / 1e6,
    cache_read_price: 0.03 / 1e6
  }
};
```
