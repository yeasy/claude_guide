...
Claude Fable 5、Opus 4.8、Opus 4.7、Opus 4.6 和 Sonnet 4.6 已进入 1 百万 Token 上下文窗口 (Context Window) 的公开规格或 beta 规格；可用性、账号权限和计费方式必须按官方模型页、迁移指南与价格页核验。按官方价格页，这些 1M 模型的整个上下文窗口按**标准单价**计费——90 万 token 的请求与 9 千 token 的请求单价相同，缓存与批处理折扣亦全程适用。Haiku 4.5、Sonnet 4.5 等其他 Claude 模型通常为 200K。这并不意味着应该无限制地往里塞东西。
