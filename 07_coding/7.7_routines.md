...
自动响应 GitHub 仓库事件。官方目前支持两类事件：**Pull request**（PR 创建/关闭/指派/打标签/同步等）与 **Release**（创建/发布/编辑/删除）。Claude 为每个匹配的事件创建独立会话，并能处理后续更新：

- PR 创建时自动进行代码审查
- Release 发布时自动生成变更摘要
- 其他事件（如 CI 失败、Issue 评论）可由 CI/监控系统经 **API 触发** 间接接入
...
| CI 失败修复 | API（CI 系统回调） | 分析失败日志，尝试修复并推送 |
