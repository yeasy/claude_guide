...
1. **云供应商路径**：如果启用了 Bedrock 或 Vertex AI，先检查对应的环境变量和云侧凭据链。
...
Claude 会自己把这条经验提炼成 `CLAUDE.md` 里的一行规则。下一次新会话开始，这条规则就已经在系统提示里了。Boris（Anthropic Claude Code 团队）形容 Claude "出奇地擅长给自己写规则"。
