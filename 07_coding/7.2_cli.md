...
| 特性名 | 启用方式 | 作用 |
| :--- | :--- | :--- |
| **KAIROS** | `ENABLE_KAIROS=1` | 始终开启的后台助手，持续监听异常 |
| **BUDDY** | `ENABLE_BUDDY=1` | 虚拟宠物（游戏化的对话）|
| **ULTRAPLAN** | `ENABLE_ULTRAPLAN=1` | 30 分钟远程协同规划会议 |

这些特性尚在内部迭代阶段。

#### 推测执行引擎

Claude Code 包含一个 **推测执行模块** 用于性能优化：

```javascript
class SpeculationEngine {
  // 预测下一步可能的操作
  predictNextTool(conversationHistory) {
    // ML 模型预测最可能的工具调用
  }

  // 在叠加文件系统中提前执行
  executeSpeculatively(toolCall) {
    // 不修改真实文件系统，而是维护"预测层"
    // 如果预测正确，立即可用
    // 如果预测错误，丢弃预测层
  }
  ...
  // 实现可逆性
  rewindIfPredictionWrong() {
    // 撤销推测执行
  }
}
```

这使得 Claude Code 能够在用户确认前提前做好准备，但仍然保留完整的回滚能力。
