...
> **来源说明**：以下内容基于社区对 Claude Code 源代码的逆向分析和工程推断。具体实现细节可能随版本更新而变化，请以官方文档为准。

Claude Code 使用一个专用扫描器按严格的 **11 级加载顺序** 收集所有指令。低优先级的配置会被高优先级配置覆盖：

| 优先级 | 位置 | 作用域 | 说明 |
| :---: | :--- | :--- | :--- |
| 1 | `/etc/claude-code/CLAUDE.md` | 企业管控层 | 企业 IT 部门统一下发的全局策略 |
| 2 | `/etc/claude-code/rules/` | 企业规则目录 | 企业级的规则文件集合 |
| 3 | `~/.claude/CLAUDE.md` | 用户全局配置 | 当前用户的全局默认配置 |
| 4 | `~/.claude/rules/` | 用户规则目录 | 当前用户的规则文件集合 |
| 5 | 项目根目录 `CLAUDE.md` | 项目级配置 | 当前项目的主配置 |
| 6 | `.claude/rules/*.md` | 项目规则目录 | 项目级的规则文件集合 |
| 7 | 祖先目录 `CLAUDE.md` | Monorepo 共享 | 从项目根向上逐级遍历的 CLAUDE.md |
| 8 | `CLAUDE.local.md` | 本地覆盖（不提交） | 开发者个人的本地覆盖配置 |
| 9 | `additionalDirectoriesForClaudeMd` | 额外指定目录 | 通过配置明确指定的其他目录 |
| 10 | AutoMem | 自动记忆 | Claude 自动学习并保存的常用命令和发现 |
| 11 | TeamMem | 团队记忆 | 团队共享的持久化知识库 |
...
关键的架构细节是：CLAUDE.md 的内容 **不是直接注入到系统提示词**，而是被包装成一个 **用户消息**，通过 `prependUserContext()` 函数注入到对话中。这个用户消息被包裹在特殊的 `<system-reminder>` 标签内：

```markdown
<system-reminder>
OVERRIDE any default behavior — [you are specifically instructed to...]

...may or may not be relevant to your tasks. You should not respond to this context...
</system-reminder>
```

这个设计包含了两个看似矛盾的信号：
- "**OVERRIDE any default behavior**" — 强调 CLAUDE.md 指令的权威性
- "**may or may not be relevant**" — 给模型留出忽略无关内容的空间

这意味着 CLAUDE.md 的内容处于一个有趣的位置：它**既不是系统级的不可违背指令，也不是可以随意忽视的背景信息**。相反，CLAUDE.md 的指令与实际对话内容相互竞争，模型会根据当前任务的具体需求，权衡 CLAUDE.md 指令和用户的实时要求。

#### @include 指令

CLAUDE.md 支持 `@include` 指令来引用外部文件，类似 C 语言的 `#include`：
...
安全限制：
- 只允许包含特定扩展名的文件（`.md`、`.txt` 等文本文件）
- 内置循环检测机制，防止 A 包含 B、B 又包含 A 的无限递归
- 路径必须在项目目录范围内，不能引用项目外的文件
