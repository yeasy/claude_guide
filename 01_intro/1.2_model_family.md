...
*   **Claude Opus 4.7 (2026.04)**: 在高难度软件工程、视觉分辨率和复杂多步任务一致性上相比 4.6 有显著提升。SWE-bench Verified 达 87.6%（4.6 为 80.8%），Terminal-Bench 2.0 达 69.4%（4.6 为 65.4%），GPQA Diamond 达 94.2%（4.6 为 91.3%），Finance Agent 达 64.4%（4.6 为 60.7%）。定价沿用 $5/$25，上下文窗口 1M token 无溢价。注意：4.7 使用新 tokenizer，同一文本可能消耗 1.00–1.35 倍 token，迁移时建议监控实际成本。
...
- **Claude Sonnet 4.6**: 公开文档明确支持 **Extended Thinking**；是否支持 Adaptive Thinking 应以当前模型能力或 release notes 为准，不要硬编码 `thinking={type: "adaptive"}`。
