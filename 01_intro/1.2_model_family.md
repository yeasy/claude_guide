...
*   **Claude Opus 4.1 (2025.08)**: 专注于 Agentic 任务和编程能力的增量升级，SWE-bench Verified 从 72.5% 提至 74.5%（+2 个百分点）。（注：Claude Opus 4 已于 2026-04-14 在 API 中 deprecated，计划 2026-06-15 retired；Claude Opus 4.1 已于 2026-06-05 deprecated，计划 2026-08-05 retired，推荐迁移到 Opus 4.8。）
...
*   **Claude Fable 5 (2026.06)**: 2026-06-09 正式 GA，是 Anthropic 能力最强的广泛发布模型，顶层命名不再沿用 Opus/Sonnet/Haiku 体系。定价 $10/$50，1M 上下文、128K 最大输出，Adaptive Thinking 常开（不支持手动 Extended Thinking），模型 ID 为 `claude-fable-5`。同日发布的 **Claude Mythos 5**（`claude-mythos-5`，Mythos Preview 的继任者）仅面向 Project Glasswing 受邀客户提供。2026-06-12 起 Anthropic 暂停两者访问；生产系统不要把它们作为默认可用模型。详见 [13.1 Claude 4.x 现状与 Claude 5 展望](../13_advanced/13.1_claude5_preview.md)。
...
以当前主力三档为例（Opus 4.8、Sonnet 4.6、Haiku 4.5），按输入价格：Opus ($5) 约为 Sonnet ($3) 的 1.7 倍、Haiku ($1) 的 5 倍；Sonnet 约为 Haiku 的 3 倍。具体倍率随版本迭代持续变化，请以附录定价表为准。
...
- **Claude Opus 4.8 / 4.7**: 仅支持 **Adaptive Thinking**（`thinking={type: "adaptive"}`），模型根据任务复杂度自动决定思考深度。不支持 Extended Thinking。
- **Claude Sonnet 4.6**: 支持 **Adaptive Thinking**（推荐）和 **Extended Thinking**；手动 `budget_tokens` 路径已弃用但仍可用。
