...
```text
~/.claude/skills/my-skill/SKILL.md      # personal：当前用户所有项目可用
.claude/skills/my-skill/SKILL.md        # project：当前项目可用，可随项目共享
```

Claude 会自动发现并加载这些 Skills。

旧的 `.claude/commands/` 文件仍然可用，并支持相同的 frontmatter；官方推荐把自定义工作流迁移为 Skills，因为 Skills 已承载 slash-command 触发能力，并额外支持脚本、资源、示例等 supporting files。
