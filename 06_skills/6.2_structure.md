...
- 哪些参考文档需要放入 `references/`、`assets/` 或 `examples/` 目录？
...
Skills、Files API 与 code execution 均已 GA，**不再需要 beta header**（仍发送旧 header 也能正常工作）；在请求中使用时只需声明 code execution 工具，并在 `container.skills` 中按官方类型引用 `skill_id` 和版本：
