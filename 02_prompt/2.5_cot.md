...
**Claude Opus 4.8**（2026-05 发布，当前 Opus 档旗舰）：与 Opus 4.7 一样仅支持 Adaptive Thinking，手动 `budget_tokens` 同样会返回 400 错误；`effort` 在所有界面默认 `high`。
...
**Claude Sonnet 4.6**：支持 Adaptive Thinking（推荐）和 Extended Thinking 手动模式；旧的 `budget_tokens` 手动预算仍可用，但已弃用，不建议新项目继续依赖。
...
此外，2026-06 发布的新一代 **Claude Fable 5**（不再沿用 Claude 4 命名体系）更进一步：Adaptive Thinking 常开且不可关闭，不支持手动 Extended Thinking；曾于 2026-06-12 暂停访问，截至 2026-07-09 已恢复，生产接入仍需先核验官方模型页。
