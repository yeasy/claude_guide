...
> **兼容性提示**：Prefill 在 Claude Opus 4.7、Opus 4.6、Sonnet 4.6 和 Sonnet 4.5 上**不受支持**，使用时会返回 400 错误。对于这些新模型，请使用 Structured Outputs（见 2.6.3 节）或系统提示词来控制输出格式。以下内容仅适用于 Claude Sonnet 4 / 3.5 等更早且仍可访问的模型。
...
除了通过 Prompt 控制外，Claude API 最新版本原生支持强制结构化输出模式。
这通过工具使用机制（Tool Use）来实现极高可靠性的格式保证：

*   **工作原理**：开发者在请求中强制要求模型调用某个特定的 “数据提取” 工具，并在 `input_schema` 中定义必须严格遵循的 JSON 架构。
*   **适用场景**：必须保证下游数据管道不会因为少了一个逗号而崩溃的场景。
*   **可靠性**：这比仅仅在 System Prompt 中请求 JSON 准确度更高，因为底层模型经过了专门的微调以满足这些 Schema。
