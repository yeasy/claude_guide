...
> **兼容性提示**：自 Claude 4.6 系列及 Mythos Preview 起，所有更新的模型（含 Opus 4.6/4.7/4.8、Sonnet 4.6、Fable 5 与 Mythos 5）均**不再支持**对最后一条 assistant 消息做 Prefill，使用时会返回 400 错误；在对话中段插入 assistant 消息不受影响。对于这些新模型，请使用 Structured Outputs（见 2.6.3 节）或系统提示词来控制输出格式。以下内容仅适用于 Claude Sonnet 4.5 / 3.5 等仍支持 Prefill 的模型。
