...
```python
from anthropic.agent import Agent, AgentTeam

class DocumentProcessingTeam(AgentTeam):
    """文档处理团队应用层"""
    ...
    async def process_document(self, doc_path: str) -> dict:
        ...
        try:
            # 步骤 1：解析
            parsed = await self.parser_agent.process({
                "file_path": doc_path
            })

            # 步骤 2：分析
            analysis = await self.analyzer_agent.process({
                "content": parsed,
                "analysis_type": "comprehensive"
            })

            # 步骤 3：总结
            summary = await self.summarizer_agent.process({
                "analysis": analysis
            })
...
```python
from anthropic.agent import AgentRuntime, MessageQueue
...
class CustomAgentRuntime(AgentRuntime):
    ...
    async def execute_agent(self, agent: Agent, input_data: dict):
        """执行单个 Agent"""
        # 1. 初始化上下文
...
class AnthropicService:
    ...
    async def batch_call(self, requests: list):
        ...
        batch_client = self.client.beta.batch
        # ...实现批量处理逻辑
```
...
class CostOptimizer:
    ...
    def get_agent_cost_estimate(self, agent_name: str, task_complexity: str) -> dict:
        ...
        input_price, output_price = model_prices.get(
            agent_name,
            model_prices["claude-3-5-sonnet"]
        )
...
class RobustAgent:
    ...
    def __init__(self, agent_id: str, model: str = "claude-sonnet-4-5-20250929"):
        self.agent_id = agent_id
        self.model = model
...
本节展示一个完整的、可运行的实际项目，演示如何使用 Agent SDK 构建一个处理多种文档的三 Agent 团队。
