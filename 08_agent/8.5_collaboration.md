...
#### Anthropic Agent SDK 与 Claude Code 子代理并联

对于更加专业或特殊的定制化工作流，可以使用 Anthropic 发布的原生 **Agent SDK** 开发你独属团队或业务的调度代理。而在底层的支持中，当你给 **Claude Code** 布置繁重的代码重构类长链任务时，它可以生成、派发甚至协调多个 **Sub-Agents (子代理)** 实现高度平行作画。主代理 (Lead Agent) 在这其中更像是总工程调度员，它可以指派独立的子代理在项目某个独立的子部分（比如独立的新开分支、独立的代码文件树下）平行验证代码，最终将成果汇总，极大地突破了传统人机编程单线并行的瓶颈。
