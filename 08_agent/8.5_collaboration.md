## 8.5 多 Agent 协作 (Multi-Agent Collaboration)
...
#### Hierarchical (层级式/老板-员工模式)
这是最常见、最可控的模式。
*   **Boss (Orchestrator)**: 负责拆解任务，分发工单，验收成果。
*   **Worker A (Coder)**: 只写代码。
*   **Worker B (Tester)**: 只写测试。

```mermaid
graph TD
    User --> Boss[Orchestrator Agent]
    Boss -->|Task: Write Code| Coder[Coder Agent]
    Boss -->|Task: Review| Reviewer[Reviewer Agent]
    Boss -->|Task: Docs| Writer[Tech Writer Agent]
    
    Coder -->|Result| Boss
    Reviewer -->|Result| Boss
    Writer -->|Result| Boss
...
*   **Shared Context**: 每个人都知道发生了什么。
*   **Turn-taking**: 需要一种机制决定下一个谁发言（轮询，或者由 LLM 决定）。
...
### 8.5.2 人工参与决策 (Human-in-the-Loop)
...
1.  **明确定义的 SOP**: 每个 Worker 的 System Prompt 必须极度具体。"你是 Python 专家，只写 Python，不要写 Shell。"
2.  **Shared State Database**: 不要把所有信息都放在 Prompt 里传。用 Redis 或文件系统作为 Agent 之间的"共享白板"。Coder 写完代码存在 `/tmp/code.py`，Tester 去读这个文件，而不是把代码贴在对话里。
3.  **Max Turn Limit**: 设置严格的熔断机制。如果 Boss 和 Worker 来回扯皮超过 10 轮，强制终止并报错给人类。
