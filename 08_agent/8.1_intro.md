...
#### Brain (LLM)
这是控制中心。在本书中，专注于 **Claude 4.5 Sonnet / Opus** 作为大脑。
Claude 的强项在于推理能力（Reasoning），这对于 Agent 面对未知错误时的自我修正至关重要。

#### Planning (规划)
人类在做复杂任务时会先列清单。AI 也是。
...
2.  **超长 Context (200k)**：Agent 在运行过程中会产生大量的日志和中间状态。Claude 的大肚量能容纳更长的执行历史，减少 "灾难性遗忘"。
...
构建 Agent 就像搭积木。虽然组件是一样的，但搭建方式（设计模式）决定了它是摇摇欲坠还是坚若磐石。
以下来看看业界最流行的几种 Agent 设计模式。
