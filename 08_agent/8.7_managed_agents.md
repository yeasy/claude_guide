...
此前，将 Agent 推向生产需要大量基础设施建设：执行环境、工具权限、会话状态、文件系统、网络访问和审计边界。**Claude Managed Agents** 是 Anthropic 提供的托管 Agent harness，运行在托管基础设施上，适合长时间运行和异步任务。它仍处于 beta，所有 Managed Agents API 请求都需要 `managed-agents-2026-04-01` beta header；具体行为和可用特性应以官方文档为准。
...
> 📖 Managed Agents 当前处于 beta，详见 [官方文档](https://platform.claude.com/docs/en/managed-agents/overview)。定价、rate limit、ZDR 覆盖范围和 research preview 能力都应按最新官方页面或合同条款确认。
