...
此前，将 Agent 推向生产需要大量基础设施建设：执行环境、工具权限、会话状态、文件系统、网络访问和审计边界。**Claude Managed Agents** 是 Anthropic 提供的托管 Agent harness，运行在托管基础设施上，适合长时间运行和异步任务。它已于 2026-05-07 正式发布；部分 multiagent orchestration / outcomes 子能力仍标注为 research preview，具体可用特性以 [官方文档](https://platform.claude.com/docs/en/managed-agents/overview) 为准。
...
> 📖 Managed Agents 已于 2026-05-07 GA，详见 [官方文档](https://platform.claude.com/docs/en/managed-agents/overview)。定价、rate limit、ZDR 覆盖范围和仍处 research preview 的子能力都应按最新官方页面或合同条款确认。
