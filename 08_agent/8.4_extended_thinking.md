...
> - **Claude Fable 5**：Adaptive Thinking 常开且不可关闭；不支持手动 Extended Thinking（`thinking={type: "enabled", budget_tokens: N}` 会返回 400 错误），思考深度用 `output_config={"effort": ...}` 控制；2026-06-12 起访问暂停，接入前需核验当前可用性。
