...
> - **Claude Sonnet 4.6**：同时支持 Adaptive Thinking（`thinking={type: "adaptive"}`）和 Extended Thinking（`thinking={type: "enabled", "budget_tokens": N}`），推荐使用 Adaptive。
...
```python
# 推荐写法：Adaptive Thinking（Claude Opus 4.7 / Sonnet 4.6）
...
*   Opus 4.7 和 Sonnet 4.6 优先使用 `thinking={“type”: “adaptive”}` 配合 `output_config={“effort”: ...}` 控制思考深度。
*   Opus 4.6 和 Haiku 4.5 仅支持 `thinking={“type”: “enabled”, “budget_tokens”: N}`（Extended Thinking），不支持 Adaptive。
