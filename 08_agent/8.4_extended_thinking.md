...
Extended Thinking 本质上仍是基于自然语言的串行推理——模型用"内心独白"逐步推导。这种范式有一个天然瓶颈：语言是低带宽的，一步一步"说"出推理过程效率较低。研究前沿正在探索两个可能打破这一瓶颈的方向：

**隐空间推理（Latent Space Reasoning）**

以 Meta 的 Coconut 研究为代表，思维链不再是语言序列，而是高维向量。一个向量可以压缩表达一整段推理，并且能像量子叠加态一样同时编码多条推理路径，效率有指数级提升的潜力。这意味着未来的"思考"可能不再需要生成数千个可读的 token，而是在隐空间中用远更紧凑的方式完成同等甚至更深度的推理。

**并行推理（Parallel Reasoning）**

当前的思维链是严格串行的——每一步依赖前一步的输出。但人类思考时常常是多线程的（比如同时权衡领导提出的 5 点要求）。并行推理让模型学会拆解独立的子问题并发执行，再合并结果，有望突破串行推理的延迟瓶颈。

这两个方向目前仍处于研究阶段，尚未进入主流产品。但它们暗示了一个重要趋势：**下一代推理能力的提升可能不再来自"想得更久"，而是来自"想得更高效"**。
