...
*   **复杂推理任务**：`effort: "high"`（默认）或 `"max"`（仅 Opus 4.6/4.7），深度推理。
*   **Agentic 编码/搜索**：`effort: "xhigh"`（Opus 4.7 的默认 effort 级别），在工具使用密集的场景下提供更充分的推理。使用 `xhigh` 时建议将 `max_tokens` 设为 64k 以上，为思考和工具调用留出充足空间。
