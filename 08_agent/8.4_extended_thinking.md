...
> - **Claude Opus 4.7**：仅支持 `thinking={type: "adaptive"}`，不再接受 `type: "enabled"` 的旧语法（会返回 400 错误）。
> - **Claude Opus 4.6 / Sonnet 4.6**：推荐使用 `thinking={type: "adaptive"}`。旧写法 `thinking={type: "enabled", "budget_tokens": N}` 仍可用但已标记弃用（deprecated），将在未来版本移除。
