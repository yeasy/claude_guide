...
```python
# 推荐写法：Adaptive Thinking（Sonnet 5 / Opus 4.8 / Opus 4.7 / Opus 4.6；Sonnet 4.6 仅作为迁移兼容性参考）
response = client.messages.create(
    model="claude-opus-4-8",
    max_tokens=8192,
    thinking={"type": "adaptive"},
    output_config={"effort": "high"},  # low / medium / high；Opus 4.8/4.7 还可用 xhigh / max
...
*   Sonnet 5 默认开启 Adaptive Thinking，不接受手动 `budget_tokens`。Opus 4.8、Opus 4.7 和 Opus 4.6 优先使用 `thinking={"type": "adaptive"}` 配合 `output_config={"effort": ...}` 控制思考深度；其中 Opus 4.8 / 4.7 不支持手动 `budget_tokens`。
...
*   **极端复杂任务/Agentic 编码**：`effort: "xhigh"` 或 `"max"`（Opus 4.8 / 4.7 可用），在工具使用密集的场景下提供更充分的推理。官方默认档位仍是 `high`；使用 `xhigh`/`max` 时建议将 `max_tokens` 设为 64k 以上，为思考和工具调用留出充足空间。
