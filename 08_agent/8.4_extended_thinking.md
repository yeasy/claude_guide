...
> - **Claude Sonnet 4.6**：公开文档明确支持 Extended Thinking（`thinking={type: "enabled", "budget_tokens": N}`）；不要把 Opus 4.6 的 adaptive 写法直接套用到 Sonnet 4.6，除非当前模型能力查询或 release notes 已明确支持。
...
```python
# 推荐写法：Adaptive Thinking（Claude Opus 4.7 / Opus 4.6）
...
*   Opus 4.7 和 Opus 4.6 优先使用 `thinking={"type": "adaptive"}` 配合 `output_config={"effort": ...}` 控制思考深度。
*   Opus 4.6 支持 `thinking={"type": "adaptive"}`（推荐），也仍兼容 `thinking={"type": "enabled", "budget_tokens": N}`（已弃用但仍可用）；Haiku 4.5 仅支持 Extended Thinking，不支持 Adaptive。
