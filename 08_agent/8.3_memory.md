...
### 8.3.2 上下文记忆 vs 权重记忆：两种本质不同的知识载体

上述三层架构解决的是"如何存取信息"的问题，但在底层还存在一个更根本的区分——信息究竟住在哪里？

LLM 的记忆可以分为两类：

- **上下文记忆（Context Memory）**：类似人的短期工作记忆，存在于每次推理的 Prompt 中。它的优势是即取即用、可动态扩展（从 200K 到 1M token），但成本随长度线性增长，且会话结束即消失。田渊栋团队的 Position Interpolation 研究证明，仅需调整位置编码并少量微调，就能以极低代价成倍扩展上下文窗口。
- **权重记忆（Weight Memory）**：类似人的长期深层记忆，通过预训练固化在模型参数中。模型发布那一刻，其"世界观"就冻结了——后续的微调只能做局部优化，无法重塑底层认知结构。

这个区分对 Agent 设计有直接影响：当 Agent 回答"错误"时，你需要判断问题出在哪一层——如果是上下文不足（该给的信息没给），可以通过改进 RAG 或扩大窗口解决；如果是权重本身的知识缺陷（模型"从来没学过"），则只能等待下一代模型或通过工具调用来弥补。

更深层的前沿挑战在于**持续学习（Continual Learning）**：让模型在推理过程中同步更新权重，且不发生"灾难性遗忘"。这比单纯扩大上下文窗口更关键，因为它触及了从"死记硬背"到"结构性顿悟"的跃迁——就像孩子从机械背诵数字到突然理解大小关系和加减运算的那个质变时刻。当前尚无突破性解法，但这被视为通向 AGI 的关键瓶颈。
