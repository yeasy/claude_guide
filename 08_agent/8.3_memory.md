...
*   **用法**: 在发送请求时，在可复用内容块上添加 `cache_control` 标记；默认 5 分钟缓存不需要额外 beta header。只有使用 1 小时缓存等扩展能力时，才需要按官方文档添加对应 beta header。
...
| **Fork（分叉）** | `claude --resume <id> --fork` | 从某个时间点创建分支，后续独立演化 |
