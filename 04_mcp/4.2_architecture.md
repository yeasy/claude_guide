...
当多个配置源（本地、项目、用户）声明了 MCP Server 时，Claude Code 的三个配置作用域**按名称**匹配重复项——同名 Server 只按最高优先级来源连接一次。按**端点**（URL 或启动命令）匹配的去重只适用于插件与连接器：当插件/连接器指向的 URL 或命令与上层某个 Server 相同时，会被视为重复。
