...
近期 Claude 4.6/4.7 的 Computer Use 组合通常是 `computer-use-2025-11-24` / `computer_20251124`，并配合 `text_editor_20250728` 与 `bash_20250124`；旧 Claude 4 / 3.7 文档曾使用 `computer-use-2025-01-24` / `computer_20250124`。版本会随模型更新，不能写死成永久规则。
...
*   **输入参数**:
    ...
    *   `text`: 要输入的文本（可选）。
*   **特性**: Claude 经过特殊训练，能够理解屏幕截图和坐标，但坐标语义取决于模型与工具版本。Opus 4.7 的较新 Computer Use 工具支持更高分辨率与 1:1 坐标；较旧工具仍可能要求缩放到官方限制范围内，生产实现应按当前 quickstart 设置分辨率和坐标转换。
...
*   **成本控制**：
    ...
    *   开启 Prompt Caching（已正式发布，支持 5 分钟和 1 小时两种缓存时长）。
