...
*   **Adaptive Thinking (自适应思考)**: Claude Opus 4.7 和 Sonnet 4.6 支持的推理模式（`thinking={type: "adaptive"}`），模型根据任务复杂度自动决定推理深度，并支持交错思考（在工具调用之间也进行推理）。与 Extended Thinking 不同，无需手动设置 `budget_tokens`。
...
*   **Extended Thinking (扩展思考)**: Claude 在处理复杂任务时的推理模式（`thinking={type: "enabled", budget_tokens: N}`），模型在后台进行长时间的隐式推理（System 2 Thinking）以提高准确率。支持 Sonnet 4.6、Opus 4.6 和 Haiku 4.5，不支持 Opus 4.7（后者使用 Adaptive Thinking）。
