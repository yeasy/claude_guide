...
```
┌─ 需要多模态吗？
│  ├─ 必须支持视频 → Gemini 2.5 Pro
│  ├─ 只需图像 → Claude 4.6 或 GPT-4o
│  └─ 不需要 → 继续
│
├─ 延迟要求是什么？
│  ├─ <200ms → Claude Haiku
...
性能评分
    │
  98├ Claude Opus 4.6 ★ (最高性能，高成本)
    │     ╱
  95├────╱─ Sonnet 4.6, GPT-5, Gemini 2.5 Pro (Pareto 前沿)
    │   ╱
  90├──╱─── Haiku 4.5, Gemini 2.5 Flash ★
    │ ╱
  85├╱───── DeepSeek-V3
    │
  80├─────────────────────────────────────
...
├─ 用途：通用 AI 应用、内容生成、代码助手
└─ 预期吞吐：20K-100K 个请求/月

预算 $1000-$5000/月
├─ 优先级：性能 > 成本
├─ 推荐模型：Sonnet 4.6 + 少量 Opus 4.6
├─ 用途：混合工作流
│  - 简单任务用 Sonnet
│  - 复杂推理用 Opus 4.6
└─ 预期吞吐：5K-20K 个请求/月
