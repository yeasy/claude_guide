...
> **关于 Claude 顶配模型口径**：1.1 节表格列出 Claude 当前主力三档（Opus 4.8 / Sonnet 4.6 / Haiku 4.5）。2026-06-09 起 Anthropic 另发布了更高一档的 **Claude Fable 5**（$10/$50、1M 上下文、128K 输出，能力最强的广泛发布模型），尚未纳入本附录的对比矩阵。本章后续的对比表、决策树与 Pareto 分析仍以 **Opus 4.7** 的具体数字代表 Claude Opus 线的能力与成本——因为 Opus 4.8 沿用 4.7 的 $5/$25 定价与 1M 上下文，能力为 4.7 的超集，故这些数字同样适用于 Opus 4.8。选型时可直接将下文“Opus 4.7”理解为“Opus 线（当前 Opus 档旗舰为 4.8）”。
...
| 厂商 | 模型系列 | 当前最新版本 | 发布时间 |
|-----|--------|-----------|--------|
| Anthropic | Claude | Fable 5（2026-06-09 GA）；Opus 4.8 / Sonnet 4.6 / Haiku 4.5 | 2026-06 |
...
graph TD
    ...
    C["性能等级 95: GPT-5.4, Gemini 3.1 Pro<br/>(严格 2D Pareto 前沿)"]
...
基于输入成本和综合性能评分（MATH + HE + MMLU 的平均值），以下区分严格 2D Pareto 前沿模型与生态内实用候选。若只看“更便宜且更高分”两个维度，Sonnet 4.6 不在严格前沿上；若限定 Claude 生态、工具链、安全对齐和中文体验，它仍可作为通用推荐。
...
| **Claude Opus 4.7** | $5.00 | 97 | 超高精度、研究级 | 右上角（最高性能） |
...
├─ vs Opus ($5/M, 98%): Sonnet 更便宜但性能更弱 → 都在前沿上 ✓
├─ vs GPT-5.4 ($2.50/M, 94%): GPT-5.4 更便宜且性能更高 → Sonnet 被支配 ✗
└─ vs Gemini 3.1 Pro ($2/M, 95%): Gemini 更便宜且性能更高 → Sonnet 被支配 ✗
