...
├─ 系统提示词 (System Prompt): 2,000 token (1%)
├─ 当前用户消息: 500 token (0.25%)
├─ 有效历史消息: 150,000 token (75%)
│   ├─ 必需背景 (前 N 条): 40,000 token
│   └─ 相关历史 (检索结果): 110,000 token
├─ 提示缓存: 30,000 token (15%) [免费/低成本]
└─ 输出空间 (Output): 17,500 token (8.75%)
```
