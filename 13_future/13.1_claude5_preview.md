...
Claude 4.6（claude-opus-4-6）代表了 Anthropic 在 2024-2025 年技术成果的集大成之作。与前代相比，其主要能力提升包括：
...
- **HumanEval 评分**：92.3%（超越 GPT-4o 的 91.5%）
...
- **幻觉率**：1.3%（相比 Claude 4.5 的 2.1%，显著降低）
...
- **处理效率**：处理效率提升 2.5 倍
- **缓存成本**：提示缓存效率提升 90%（从 $3.00/百万输入 token 降至 $0.30/百万缓存读取 token）
- **文档处理**：能够在单个请求中处理 5 个高清 PDF 文档的完整内容

#### 13.1.2 Anthropic 当前模型梯队

Anthropic 维护的 Claude 模型系列包括三个不同规模的模型，形成了三层的梯队结构：

**上一代 Claude 4.5 Sonnet（通用高性能模型）**
...
- 吞吐量：支持并发请求数 50K+/秒
- 成本：$1.00/百万输入 token，$5.00/百万输出 token
...
- 成本：$5.00/百万输入 token，$25.00/百万输出 token
- 最佳使用场景：最复杂的推理任务、研究级别的分析、需要创意突破的问题

#### 13.1.3 能力对标与基准测试数据

| 能力维度 | Claude 4.5 Haiku | 上一代 Claude 4.5 Sonnet | Claude Sonnet 4.6 | GPT-4o | Gemini 2.5 Pro |
|---------|-----------------|------------------|-----------|--------|---------------|
...
- 成本：$1.00/$5.00 per 百万 token，每个客服请求平均成本 $0.0015-0.004
...
选项 A: 全部用 Haiku
- 输入 token 平均：200，输出平均：150
- 成本：100 万 × 200 input × $1.00/M + 100 万 × 150 output × $5.00/M = $200 + $750 = 约 $950/月

选项 B: 80% Haiku + 20% Sonnet（复杂问题）
- Haiku 部分：80 万 × 200 input × $1.00/M + 80 万 × 150 output × $5.00/M = $160 + $600 = $760/月
- Sonnet 部分：20 万 × 250 input × $3.00/M + 20 万 × 250 output × $15.00/M = $150 + $750 = $900/月
- 总成本：$1,660/月
...
def optimize_with_caching(documents: list[str], queries: list[str]) -> float:
    ...
    cache_input_price = 3.75 / 1_000_000  # 缓存写入
    cache_read_price = 0.30 / 1_000_000   # 缓存读取
