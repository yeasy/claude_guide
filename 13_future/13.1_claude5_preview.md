...
预训练 Scaling Law（增大数据和参数量 → 性能持续提升）依然有效，但边际递减日趋明显。大厂坚持这条路在很大程度上是路径依赖——团队基建已就绪，堆算力和数据是风险最低的策略。然而这条路正面临物理限制：电力供给、存储瓶颈（随参数从 70B 飙升至 500B 乃至 1T，内存成为最紧缺资源）以及卡间通信延迟。

推理端的 Scaling Law（Test-Time Scaling，增加推理时间提升效果）同样存在天花板：强化学习的能力上界被预训练所锁定。强化学习本质是"搜索放大器"，从预训练提供的候选路径中找到正确解。如果预训练根本没见过某类问题，连候选路径都不存在，搜索便是在空集中进行。因此，推理能力的提升必须建立在扎实的预训练基础之上。

突破这一瓶颈的关键方向是**持续学习（Continual Learning）**——让模型在部署后仍能持续吸收新知识、更新权重，且不发生灾难性遗忘。这将从根本上解决"模型发布即知识冻结"的问题。

蒸馏（Distillation）技术的普及也在重塑竞争格局：较弱模型通过学习更强模型的输出，能迅速逼近领先水平，导致技术窗口期被极度压缩。在护城河排序上，业内共识正在形成：**数据 > 基础设施 > 算法 > 人才**。垂直领域的稀缺数据是硬约束，而算法创新因人才流动（新方案提出后 2-3 个月便传遍行业）难以构成长期壁垒。
